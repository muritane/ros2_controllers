...
TEST_P(TrajectoryControllerTestParameterized, foo)
{
  rclcpp::executors::SingleThreadedExecutor executor;
  std::vector<rclcpp::Parameter> params = {
    rclcpp::Parameter("open_loop_control", true)};
  SetUpAndActivateTrajectoryController(executor,  params, true);

  // goal setup
  double goal = 11.0;
  // std::vector<double> goal_positions = {11.0, 0.0, 0.0};
  // std::vector<double> reference_goal = {11.0, 0.0, 0.0};
  // double state_from_command_offset = 0.3;

  // send msg
  constexpr auto FIRST_POINT_TIME = std::chrono::milliseconds(50);
  builtin_interfaces::msg::Duration time_from_start{rclcpp::Duration(FIRST_POINT_TIME)};
  // std::vector<std::vector<double>> points{{goal}};

  auto clock = rclcpp::Clock(RCL_STEADY_TIME);

  size_t pub_iterations = 3;

  for (size_t pub_i = 0; pub_i < pub_iterations; ++pub_i) {
    double curr_goal = goal / (pub_iterations - pub_i);
    RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "pub_i: %zu, curr_goal: %f", pub_i, curr_goal);

    // std::vector<double> goal_positions = {curr_goal, 0.0, 0.0};
    std::vector<std::vector<double>> pos_points{{curr_goal, 0.0, 0.0}};

    publish(time_from_start, pos_points, rclcpp::Time(0.0, 0.0, RCL_STEADY_TIME));
    traj_controller_->wait_for_trajectory(executor);
    double controller_update_wait_time = 0.01;
    ...
    for (size_t i = 0; i < iterations; ++i) {
      ...
      auto current_state_when_offset = traj_controller_->get_current_state_when_offset();
      state_when_offset += std::to_string(current_state_when_offset.positions[0]) + ", ";
      // publish(time_from_start, points, rclcpp::Time(0.0, 0.0, RCL_STEADY_TIME));
      // updateController(rclcpp::Duration::from_seconds(controller_update_wait_time));

      auto now = clock.now();
      stamps += std::to_string(now.seconds()) + ", ";

      traj_controller_->update(now, rclcpp::Duration::from_seconds(controller_update_wait_time));
      clock.sleep_for(rclcpp::Duration::from_seconds(controller_update_wait_time));
    }
    joint_pos += std::to_string(joint_pos_[0]) + "]";
    auto current_state_when_offset = traj_controller_->get_current_state_when_offset();
    state_when_offset += std::to_string(current_state_when_offset.positions[0]) + "]";
    auto now = clock.now();
    stamps += std::to_string(now.seconds()) + "]";
    RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "joint_pos: %s", joint_pos.c_str());
    RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "state_off: %s", state_when_offset.c_str());
    RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "tt_stamps: %s", stamps.c_str());
